...
# # Register your custom objects here
...
# def deepfakes_video_predict(input_video, alpha=0.6):
#     """
#     Predict whether a video is DeepFake using both image (frames) and audio.

#     Arguments:
#         input_video {str} -- Path to video file.
#         alpha {float} -- Weight for video prediction when combining with audio (0-1).
#                          Audio weight will be 1-alpha.

#     Returns:
#         str -- Final decision text with confidence.
#     """
...
#         pred = model.predict(np.expand_dims(face_norm, axis=0))[0]
...
#     audio_pt = torch.Tensor(y).unsqueeze(0)
    
#     with torch.no_grad():
#         audio_logits = load_audio_model()(audio_pt)
    
#     audio_pred = torch.softmax(audio_logits, dim=1).numpy()[0]  # Get probabilities
#     audio_real_prob, audio_fake_prob = audio_pred[0], audio_pred[1]

#     # ------------------------
#     # Combine predictions
#     # ------------------------
#     combined_fake = alpha * fake_mean + (1 - alpha) * audio_fake_prob
#     combined_real = alpha * real_mean + (1 - alpha) * audio_real_prob

#     if combined_fake > combined_real:
#         final_text = f"FINAL DECISION: FAKE\n" \
#                      f"Video Confidence: {round(fake_mean*100, 2)}%\n" \
#                      f"Audio Confidence: {round(audio_fake_prob*100, 2)}%"
#     else:
#         final_text = f"FINAL DECISION: REAL\n" \
#                      f"Video Confidence: {round(real_mean*100, 2)}%\n" \
#                      f"Audio Confidence: {round(audio_real_prob*100, 2)}%"
